...
export default function HoldingRow({ h, onUpdated }: { h: Holding; onUpdated: () => void }) {
  ...
  const [ticker, setTicker] = useState(h.ticker)
  ...
  const resetAndClose = () => {
    setTicker(h.ticker); setName(h.name); setQty(h.quantity)
    setTotalAmount(Math.round(h.quantity * h.avgPrice)); setMemo(h.memo || '')
    setEditing(false)
  }
