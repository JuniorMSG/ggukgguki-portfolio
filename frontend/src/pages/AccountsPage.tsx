...
export default function AccountsPage() {
  ...
  const handleAddCash = async () => {
    ...
    await fetch('/api/cash-assets', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userId: USER_ID, name: newCash.name, category: newCash.category, balance: Number(newCash.balance), interestRate: Number(newCash.interestRate || 0), memo: newCash.memo || null })
    })
  ...
  const INTEREST_TAX_RATE = 0.154 // 이자소득세 15.4%
                      ...
                      return (
                        <tr key={c.id} className="border-b border-gray-50 bg-blue-50/30">
                          ...
                          <td className="py-2 text-right whitespace-nowrap">
                            ...
                            <button onClick={async () => { await fetch(`/api/cash-assets/${c.id}`, { method: 'DELETE' }); setEditingCashId(null); setRefreshKey((k) => k + 1) }}
